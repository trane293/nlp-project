...
print('opening another version of input file to add to translation model..', file=sys.stderr)
inp_file2 = open('../data/input')

french = [tuple(line.strip().split()) for line in inp_file2.readlines()]

print('adding to translation model all unseen words', file=sys.stderr)
...
inp_file2.close()
print('translation model ready..', file=sys.stderr)
...
for sentence_orig in inp_file:
    ...
    try:
        ...
        print('using eta {}'.format(eta), file=sys.stderr)
        ...
        for i in range(0, len(sentence)-1):
            ...
            for q_hash, alpha in beam_output.items():
                ...
                for valid_phrase in ph_output:
                    q_new = next(createQfromHashable(q_hash, alpha), valid_phrase, eta=eta)
        ...
        elif Q_main[len(sentence)-3] != {}:
            print('getting fourth last..', file=sys.stderr)
            end_point = getBestPerformerStateHash(Q_main, len(sentence) - 3)
        else:
            print('getting fifth last..', file=sys.stderr)
            end_point = getBestPerformerStateHash(Q_main, len(sentence) - 4)
