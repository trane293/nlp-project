...
# Pw = Pdist('/home/ska196/count_1w.txt')
# Pw2 = Pdist('/home/ska196/bigram_counts')
            ...
            while heap:
                ...
                for key in Pw:
                    if sub_utf8line.startswith(key):
                        ...
                        if(probability == None):
                            probability = np.log2(lmda*Pw(key) + (1-lmda)*(0.8/(float(Pw.Size()) + float(Pw2.Size()) ) )) + head.get_item('log_prob')
                        else:
                            probability = np.log2( 
                                lmda*Pw2(head.get_item('word').decode('utf-8') + " " +"".join(key))
                                / Pw(head.get_item('word').decode('utf-8')) 
                                + (1-lmda)*Pw(head.get_item('word').decode('utf-8') )
                                #Pw(head.get_item('word').decode('utf-8') - (1-lmda)*head.get_item(log_prob) ) 
                                ) + head.get_item('log_prob')
                ...
                if num_observ == 0 and len(sub_utf8line) > 0:
                    ...
                    probability = np.log2(  1/(float((Pw.Size()) + float(Pw2.Size())))) + head.get_item('log_prob')
